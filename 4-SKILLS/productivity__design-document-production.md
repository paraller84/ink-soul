...
| **H** | **数据结构与API差距分析** | 原型设计后的数据就绪度验证——逐页逐字段核对现有表/列/API是否支撑，标注缺口并给出修复方案。实施前的强制前置门禁 | HTML 信息图 |
...
### Phase B3: 用户测试 → 反馈 → 迭代更新

当用户测试所有原型后给出结构化反馈时，走快速迭代流程：

#### 步骤 1: 解析用户反馈
用户可能针对特定页面给出具体调整要求。对每条反馈确认：
- **涉及哪个原型页面**（通过页面名称/功能描述识别）
- **修改内容**（布局/交互/新增功能/删除功能）
- **是否涉及新页面创建**（如用户要求新增"拍照批改作业"功能）

#### 步骤 2: 更新对应原型文件
- 直接修改对应的独立 HTML 文件，不重新生成全部
- 保持所有原型之间的导航链路一致（如果一个原型新增了按钮，确保链接目标存在）
- 如果是新增原型页面，创建新 HTML 文件并更新入口页面的导航

#### 步骤 3: 重新上传 + 发送新链接
- 上传所有修改过的和新创建的原型文件（用 upload_all API）
- 将更新的链接和修改说明一并发送到对话
- 不需要重新创建飞书文档，仅发送文件链接即可

#### 实战示例
本模式在 2026-05-18 AI 家庭教师家长端原型迭代中使用：
1. 用户测试了 10 个原型后给出 4 条反馈（exams/题库/练习卷/语文主页）
2. 更新了 4 个原型文件 + 创建了 1 个新页面（拍照批改作业）
3. 重新上传全部 11 个文件，发送新链接
4. 迭代周期：从用户反馈到交付完成约 20 分钟
...
### 辅助技巧：配色方案对比预览（Design Decision Support）

当用户要求**调整配色**（而非重写布局）时，采用"预览后选择"策略：

1. 保持现有布局结构**完全不动**，只生成一份独立的**对比预览HTML**
2. 在该HTML中同时显示2-3套配色变体（三列并排或卡片切换）
3. 每套方案附：色值表 + 护眼指数 + 风格关键词 + 受众匹配度
4. 上传飞书让用户选择，确认后再修改实际模板

**模板结构（三列并排对比）：**
```html
<div class="row-3">
  <!-- 方案A -->
  <div class="phone theme-a">
    <div class="label">🌌 方案A · 宇宙深蓝</div>
    <div class="hero">...复用页面骨架...</div>
  </div>
  <!-- 方案B -->
  <div class="phone theme-b">
    <div class="label">🌿 方案B · 翠绿森林</div>
    <div class="hero">...复用页面骨架...</div>
  </div>
  <!-- 方案C -->
  <div class="phone theme-c">
    <div class="label">🔥 方案C · 暖橙活力</div>
    <div class="hero">...复用页面骨架...</div>
  </div>
</div>
<!-- 色值对比表 -->
<table>
  <tr><th>特征</th><th>方案A</th><th>方案B</th><th>方案C</th></tr>
  <tr><td>背景色</td><td><code>#070d1a</code></td>...</tr>
</table>
```

**核心规则：**
- ❌ 不要直接改production模板 → 用户没选择之前改了白改
- ❌ 不要只发文字描述色值 → 用户无法视觉对比
- ✅ 对比HTML必须让用户看到布局+色值完整渲染效果
- ✅ 附颜色psychology解释（为什么适合目标受众）
- ✅ 色值注册到 `visual-style-registry` 以备未来复用
...
当用户原型设计（类型A/D）经顾问团评估后、进入C005实施前，需要**验证数据就绪度**时使用。输出为1个自包含HTML，以表格+SQL片段+函数签名+缺口标记为主，不包含phone/pad框架。

**典型触发链（多阶段设计管线）：**
```
用户需求 → 类型A/类型D 原型设计 → 顾问团评估(模式C Lightweight)
  → 类型H 数据差距分析 → 类型A 综合设计说明书 → C005 实施
```
...
| 用户用语 | 示例 |
|:---------|:------|
| "分析数据结构和API是否支撑" | "结合原型设计，分析当前数据结构和API是否支撑页面原型功能" |
| "数据就绪度" / "差距分析" | "看看现有表能不能支撑这些页面" |
| "逐页核对" | "每个页面需要哪些数据，现有表是否都有" |
...
```bash
# 1. 所有表名
sqlite3 instance/tutor.db ".tables"
# 2. 每张表的完整列定义
for t in $(sqlite3 instance/tutor.db ".tables"); do echo "=== $t ==="; sqlite3 instance/tutor.db "PRAGMA table_info($t);"; done
# 3. 现有路由和API函数
grep -rn "def \|\.route" routes/*.py | grep -v "__"
# 4. 现有模板清单
ls templates/student/
```

### Phase H2: 逐页数据映射

对每个原型页面，建立数据需求→数据源的双向映射表。**核心原则**：每行一个数据项，不合并。状态分3种：✅ 现有表/列/函数直接支撑 / ⚠️ 已有表但缺少聚合函数 / ❌ 需要新增表或列。

### Phase H3: 缺口分类与修复方案

| 缺口类型 | 修复方式 | 示例 |
|:---------|:---------|:------|
| 缺失聚合函数 | 在 library_service 中新增 | `get_weekly_checkins()` |
| 缺失 SQL 查询逻辑 | 在路由中补充或新增 service 函数 | `get_lesson_learning_status()` |
| 数据不完整 | 运行修复脚本 | lesson_number NULL → fix_null_lesson_number.py |

### Phase H4: 去重策略统一规范（高频陷阱）

当同一词语因多轮生成存在多条 question_bank 记录时，所有 COUNT 聚合必须遵循去重策略：

| 场景 | 去重粒度 | SQL 模式 |
|:-----|:---------|:---------|
| 练习次数统计 | 按 schedule_id | `COUNT(DISTINCT pr.schedule_id)` |
| 题库题量统计 | 按 answer | `COUNT(DISTINCT qb.answer)` |
| 取唯一题目ID | 按 answer 聚合并取 MIN(id) | `SELECT MIN(id) FROM question_bank GROUP BY answer` |
...
3. 需新增函数清单（含精确接口签名）
4. 需修改路由清单 + 需新增模板清单
5. 数据缺口修复方案（lesson_number NULL 修复等）
6. 实施计划（分阶段 + 工时评估）
7. 顾问团审定意见汇总
```
...
差距分析产出后，应自动触发 C007 顾问团模式C Lightweight 评估，由三角色（教育/UX/架构）评审分析完整性，将反馈迭代入 v1.1。典型审定意见类型：🏗️ 架构师关注修复链路和去重策略；🎓 教育专家关注展示优先级和称号选型；🎨 UX专家关注空状态和性能影响。

### 综合设计说明书（可选阶段）

在顾问团审定通过后，将原型设计 + 差距分析 + 审定意见合并为1份综合设计说明书（类型A）：包含页面原型、数据映射矩阵、API设计、路由设计、修复方案、实施计划、评审意见。作为进入C005实施的完整输入。

### 实战案例

参见 `references/student-prototype-pipeline-case-20260521.md` — AI家庭教师学生端从6点需求→原型v2.0→顾问团评估→差距分析v1.0→审定v1.1→综合设计说明书v1.0的完整多阶段管线。
...
> **🔴 回复长度规则（2026-05-20 用户纠正）：**
> 当 HTML 方案文档已通过飞书链接交付时，回复消息**不得重复或赘述方案内容**。只需简短确认 + 链接即可。
> ❌ 错误模式：上传HTML后还在消息中复述方案的核心内容、表格、章节摘要等
> ✅ 正确模式：`已完成。📎 链接：xxx` + 一句话总结
> 理由：用户已在飞书中阅读HTML文档，不需要我在消息中再写一遍。
...
### Phase 0A: 大规模需求审计与分期规划

**触发条件**：用户一次性提交大量系统优化需求（如 30+ 项，覆盖多个模块），要求你分析完整性、可行性，再开始设计。

**不适用**：用户只要求改 1-3 个小功能或只要求直接出原型，跳过此 Phase。

#### Step 1: 逐项对照现有系统审计

将用户需求拆解为**子项清单**，每项对照现有系统回答三个问题：

| 问题 | 检查方法 |
|:-----|:---------|
| ⚡ 数据是否存在？ | 查询数据库表名、行数、字段 |
| 🖥️ 页面/路由是否存在？ | grep 路由定义 + 模板文件清单 |
| ✅ 功能是否完整？ | 对比用户描述 vs 现有代码逻辑 |

**审计表格模板：**

```
| 需求项 | 现有状态 | 数据层 | 展示层 | 缺漏 | 难度 |
|:-------|:---------|:-------|:-------|:-----|:----:|
| 2.1 顶部积分 | ⚠️ 有但需增强 | 27条交易 | dashboard.html已有 | 缺今日变化 | ⭐⭐ |
| 2.3.1 古诗管理 | ❌ 无 | 缺古诗表 | 无路由 | 需新建 | ⭐⭐⭐ |
```

#### Step 2: 识别设计缺漏与待澄清项

将缺漏分为三类：
- **数据层缺失** — 需要建新表（如古诗表、提示词持久化表）
- **交互缺失** — 用户未明确的交互模式（如悬停 vs 点击展开）
- **架构缺失** — 未考虑的约束条件（如多子女、权限、回滚）

对每项标注建议方案和风险等级。

#### Step 3: 分期方案（P0/P1/P2）

按依赖关系和风险评分给出三期：

| 阶段 | 原则 | 典型范围 | 预估工作量 |
|:----:|:------|:---------|:----------:|
| **P0 速赢** | 数据就绪 + 改动小 + 立即可见 | 登录优化、积分增强、仪表盘入口 | 2-3 天 |
| **P1 核心** | 核心功能 + 数据层扩展 | 语文管理主页、导入增强、题库管理 | 5-7 天 |
| **P2 完整** | 新功能 + 暂缓功能 + 预留 | 学习日历、AI评语、数学/英语预留 | 5-7 天 |

#### Step 4: 决策捕捉

用户可能以**编号列表**形式回应（如「1、作为课文一类；2、练习卷...；3、不考」）。**每条决策应即时映射到方案中**，更新分期和依赖：

| 决策 | 影响 | 动作 |
|:-----|:------|:------|
| 古诗→课文一类 | 省去 1 张新表 | 复用 lesson_type 字段 |
| 手动生成练习卷 | 需新增 UI | 移到 P1 |
| 不考虑多子女 | 简化所有模板 | 锁定 student_id=1 |

**规则**：用户给出简洁决策后，立即更新方案并推进，不要追加追问。编号决策格式（1、2、3、）表示用户已想清楚，不需要再澄清。
...
| 模式 | 用户用语 | 原型范围 | 典型数量 |
|:-----|:---------|:---------|:---------|
| **P0先行** | "先做/先出原型" "P0" | 仅 P0 速赢页面的原型 | 3-5 个 |
| **全量补齐** | "补齐" "全部" "一次性出" | 所有已决案页面 | 8-15 个 |

**P0先行模式：**
- 只创建 P0 对应的交互原型
- 用户确认 P0 原型后，才开始 P0 编码
- P1/P2 暂不创建原型，仅保留 HTML 信息图文档描述

**全量补齐模式（用户说"补齐"时）：**
- 创建**需求涉及的所有页面**的原型，一个不能少
- 用户的"补齐"默认指全量页面，而非仅 P0 或前几页。即使是先做了 P0 再被要求补齐，也要一次性出齐全部页面
- 用并行委派（delegate_task 或批量 write_file）加速
- 所有原型必须保持视觉一致（共用 CSS 主题变量）
- 原型之间要有导航链路关系，形成可走通的点击流

**两种模式的共同纪律：**
- 原型中不得出现需求中没有的内容
- 所有 prototype 文件放在 `prototypes/<phase>/` 目录下
- 上传飞书后再发送链接到对话

### ⚠️ 原型制作规范（2026-05-18 用户纠正）

**原型必须严格匹配需求文本，不添加任何需求中没有的内容。**

| 违规行为 | 后果 | 用户原话 |
|:---------|:-----|:---------|
| 添加需求中没有的功能模块 | 用户指出并要求删除 | "家长中心原需求中并没有今日待办" |
| 添加需求中没有的入口按钮 | 需要重新生成 | "同时也没有练习卷和内容导入的功能" |
| 只做部分页面 | 用户要求全部补齐 | "本次修改原不止这三个页面吧" |

**实施原则：**
1. 原型中的每个元素都应在需求文本中有明确依据
2. 不要因为觉得"好看"或"方便"就添加需求中没有的功能
3. 如果有你认为需求遗漏的功能，应在审计阶段提出（Phase 0A Step 2），不可自行加入原型
4. 需求中明确说"后续再建设"的模块（如数学/英语），在原型中标记「建设中」或灰色禁用态

#### 实战案例

参见 `references/p0-planning-audit-case-20260518.md` — AI 家庭教师家长端 37 项需求审计的完整过程：系统扫描→逐个对标→3 项缺漏→4 项待决策→P0/P1/P2 分期→3 个 P0 原型→飞书交付。
...
### Phase 6: 基于顾问团评估的快速迭代管线（v2.1 新增）

当设计文档已经过 C007 顾问团评估并产出 P0/P1/P2 综合决策表后，采用**目标修补**迭代模式：

#### 工作流

```
Step 0: 确认用户指令 —— 「请根据评估意见完成页面原型设计」（或类似）
Step 1: 对 P0 项逐一执行目标 patch
Step 2: 版本递增（vN.0 → vN.1）
Step 3: 重新上传飞书（不覆盖旧版本）
Step 4: 发送新链接
```
...
| 变更类型 | 推荐的修补方法 | 注意事项 |
|:---------|:--------------|:---------|
| 文字/标签/版本号 | `patch()` 单行模式 | 用足够唯一的上下文确保匹配 |
| 导航/重复结构（多处相同） | `patch(replace_all=True)` | 确认同一字符串在所有位置都要改 |
| 整个数据块替换（段落/列表/卡片） | `patch()` 带大段上下文 | 包含目标块前后各 1-2 行确保唯一性 |
| 新增结构块（进度条、新卡片） | `patch()` 用目标位置的结束符作为锚点 | 先找到文档中唯一的结束字符串 |
| 删除大块内容 | `patch()` old_string=目标块, new_string='' | 确保不破坏 HTML 结构完整性 |

#### 不适用场景（走全文重写或子代理）

- 新增/删除整个 phone/pad 页面框架
- 重新排列页面顺序
- 大规模 CSS 主题变更

#### 验证清单

- [ ] 所有 P0 项都找到了对应的 patch 点
- [ ] 版本号已递增
- [ ] HTML 结构完整（没有截断的标签）
- [ ] 新版本上传后可以被预览
...
参见 `advisory-council-workflow` skill 的「设计文档快速迭代管线」节的实战案例。
...
> 详见 `references/prototype-alignment-workflow.md` — 单页对齐：当用户提供一个设计原型 HTML 文件，要求将当前运行页面与之对齐时使用。
> 
> 详见 `references/prototype-alignment-workflow.md` — 生产 → 原型对齐工作流（单页）及多页面全量审计（完整原型清单对比节）

**定位**：本 skill 的所有"原型→生产"流程都是**正向**的（设计文档→原型→生产）。但用户也可能在系统已上线后要求"按某个原型改"——此时需要走反向流程（原型结构分析→差异矩阵→后端数据补全→模板重写），见参考文件。

**触发时机**：当用户说"页面和原设计方案不一致""这个是老的页面""按这个附件改""检查所有页面"时。

### Step 0: 服务健康性检查（2026-05-21 新增）

**不需要立即修改代码。** 在打开任何模板文件之前，先确认服务进程是否正确运行。

**原因**：模板和路由修改后，如果旧的服务进程没有重启，用户看到的仍是旧进程在内存中加载的 Python 路由代码（即使 Jinja2 从磁盘读取了新模板，旧的路由逻辑仍可能导致页面表现不一致）。
...
ss -tlnp | grep <PORT>
# ✅ 单 PID = 正常
# ❌ 多 PID（如两个 python 进程都绑定 5003）= 内核轮转分发，~50% 请求落到旧进程
```

#### 修复

```bash
fuser -k <PORT>/tcp    # 杀死所有绑定该端口的进程
sleep 2
# 重新启动（用 Hermes 的 terminal background=true 以跟踪生命周期）
ss -tlnp | grep <PORT> || echo "✅ Port free & single PID"
```

> 详细原理见 `system-service-reliability`→「Flask Dev Server Lifecycle & Dual-Process Pitfall」节

#### 验证

修复后再次加载目标页面，确认不再是"老的页面"。如果问题仍在，再开始代码层面的差异分析。

### ⚠️ "已完成"误报陷阱（2026-05-20 用户纠正 — 反复出错模式）

**这是本 skill 管辖区内最危险的陷阱——我多次犯过模式相同的错误：**

| 日期 | 我说了什么 | 实际发生了什么 |
|:-----|:-----------|:--------------|
| 5/18 | "P0三份原型已完成" | 缺页面 + 混入需求中没有的内容 |
| 5/20 | "已按原型重写dashboard" + "审计完成" | 审计没有载入原型文件；其他页面还有大量不一致 |
| 5/20 | 静态代码审计→报告5个小问题 | 用户指出"没按原有设计页面做比对"；漏报了3页完全缺失 |

**根源：** 每次只改了当前对话直接涉及的1-2个文件，就宣告"已完成全部改造"。没有从设计方案中提取完整页面清单，没有逐页对照验收。

**强制门禁（违反将导致用户指正返工）：**

在"检查所有页面"或"按设计方案改造"的任务中，**宣告"已完成"之前，必须完成以下全部步骤**，缺一不可：

```
Step 1: 确定完整页面清单
  从原型文件列表、设计文档索引或飞书文件夹中提取所有页面，而非仅当前对话涉及的1-2页。

Step 2: 下载所有原型HTML文件
  使用 Feishu Drive API 下载每个文件（curl -L 获取的是飞书包装页，需用 /download 端点获取实际 HTML）。
  不要依赖注册表的缓存token，先 resolve_folder_token() 确认最新位置。

Step 3: 创建对照矩阵
  每行一个原型页面，列：页面名称 | 对应路由 | 原型功能摘要 | 当前实现状态 | 差异
  缺失的页面标记为 ❌ 不存在，不可跳过不报。

Step 4: 逐页修复
  按差异矩阵的优先级，每修复一个确认一个，不跳步。

Step 5: 逐页验证
  每页至少检查：200 OK + 所有关键元素存在 + 旧元素已移除。
  全部行标记为 ✅ 后，才可宣告"已完成"。
```
...
□ 我是否下载了所有原型文件（而非只看代码）？
□ 我是否为每个原型创建了对照行？
□ 未完成/缺失的页面是否明确标记为 ❌ 而非跳过？
□ 我的"已完成"是否仅限于已逐页验证的部分？
```

## 配色方案提交流程

当用户要求调整UI配色时（尤其儿童教育产品），先确认方向偏好再出方案。

**方向判断**：用户说"适合小学男生审美和防止视觉疲劳"——这既不是深色也不是浅色。关键判断是"防止视觉疲劳"通常指向浅色（低对比度背景），而非暗色（增加眼睛调节负担）。对儿童/家长端产品，浅色明快优先。

**提案流程**（两轮制）：第一轮出方向（浅色+深色各一套），第二轮在已确认方向上出三套变体。

**实战教训** 2026-05-20：首次出了3套暗色方案被拒（"建议不采用深色布局"），二次改为浅色后选定。根因是误判——将"防疲劳"理解为低调暗色，实际用户要的是低对比度浅色。


## 参考

- `prototype-alignment-workflow.md` — 生产 → 原型对齐工作流（单页 + 多页全量审计）
