...
### 专题讨论摘要（v1.4 — 2026-05-19 新增）

会议笔记处理除待办提取外，新增**专题话题识别与关键要点提取**：

- 纯启发式算法（无LLM调用，仅标准库）
- 输出到通知文本的「📋 专题讨论摘要」区块
- 同时写入 PENDING 列表的 `topics` 字段

**算法流程**:
1. **分话轮** — 正则匹配 `说话人N：` `姓名：` 等标记，合并同人连续发言
2. **话题边界** — 检测过渡词（`接下来/关于/另外一个`）或内容漂移（关键词交集 <20%）
3. **专题判定** — 时长占比≥25%**或**交锋≥3轮**或**参与者≥3人 → 标记为专题话题
4. **要点提取** — 移除填充词（嗯/哦/好的/明白），归类为观点陈述/分歧点/结论/决策
