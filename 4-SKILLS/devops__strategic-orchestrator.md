...
> 本地模型（qwen2.5:3b / qwen3:8b / qwen3.5:9b）已退役为主力，仅视觉（qwen3-vl:8b）和嵌入（qwen3-embedding）因无云端替代保留本地。
...
本地模型 = 辅助资源（Ollama @ localhost:11434）
 ├─ qwen3.5-256k → 视觉理解 + 256K上下文（新一代旗舰，一次加载即支持图文混合任务）
 ├─ qwen3-vl:8b → 视觉备用（降级）
...
| 类型 | 定义 | 举例 | 特征 |
|:----|:-----|:-----|:------|
| 🟢 **模块（Module）** | Edu-Hub 统一教育平台下的**学科模块** | C003数学 / C008英语 / C011语文 / C012飞书待办 | 共享同一 Flask app（app.py）、认证、质量门禁，但数据与业务逻辑分离 |
| 👤 **角色/方法论（Work Mode）** | 我在特定任务中**扮演的专业身份** | C004战略指挥官 / C007顾问团 / C005编码工作流 / C015质量门禁 | 我一人分饰多角，按流程行动，不产生独立Agent |

**关键理解**：
- C003/C008/C011 是 Edu-Hub 下的**学科模块**——共享同一个 Flask 应用（app.py）、认证、session、质量门禁
- C001/C010/C012 是**独立系统**——有独立代码、cron、数据持久化
...
| **LV** | 图像 | 视觉理解/OCR | qwen3.5-256k（本地，图文一体）→ qwen3-vl:8b（备）| 本地 |
...
[战略指挥官] 分析任务：
  0. 识别消息来源群
     ├─ 如为 Feishu 群聊，读取 chat_id
     ├─ 匹配群感知角色预设（feishu-chat-group-management skill）
     ├─ 若命中顾问团群（🎓教育优化/📊汇报材料/💡思想碰撞）
     │  → 自动加载对应角色阵容，进入 C007 模式
     └─ 若命中工作流群（📋会议处理/✍️内容生成/📬邮件协作）
        → 直接加载对应工作流 skill，跳过路由分级
  1. 评估上下文大小（token量）
  2. 评估推理深度（简单 vs 复杂）
  3. 判断是否涉及视觉
  ↓
  ...
  │   └─ 超过128K的任务自动路由到此层
  │
  ├─ LV → qwen3.5-256k（原生 vision + 256K，一次加载即可处理图文混合任务）
  │    qwen3-vl:8b 作为备用
  │
  └─ 混合 → 分解后按子任务路由到不同层级
  ↓
...
### 模式D：视觉任务
```python
# 调用 qwen3.5-256k（本地，图文一体模型，含原生 vision 能力）
# 一次加载即可处理图文混合任务，无需切换模型
# 降级方案：qwen3-vl:8b 作为备用
```
...
auxiliary:
  ...
  vision:         qwen3.5-256k 主 / qwen3-vl:8b 备（本地）
...
**旧行为**：当用户问"系统能不能做X"时，凭环境变量值或印象直接回答。

**典型事故**（2026-05-12）：用户问"群聊中是否需要@机器人"，我看到 `FEISHU_GROUP_POLICY=open` 就回答"不用@"，但实际 Gateway 代码中还有一个独立的 `_should_accept_group_message()` 方法执行 @mention 门控检查，open 模式下仍需 @。

**新行为**：所有"系统能力/能不能/是否支持"类问题，必须遵循：
1. 如果有相关 skill → 先加载 skill（`skill_view`）
2. 在 skill 中查找明确记载的答案
3. 如果没有 skill 或不包含所需信息 → 直接读代码确认（`terminal` + `grep`）
4. 给出保守回答：如果不确定，说"我需确认一下"

**标记为系统级检查项**：回答"能/不能"类问题必须附代码证据或在skill中有明确记载，不得凭印象。

### ❌ 在信息不完整时构建无法验证的分析框架

**旧行为**：用户问「帮我分析我的精力分配」时，构建了「6个渠道的注意力模型」，但用户指出「你没有办法掌握我所有的精力分配情况」，因为你无法看到私聊、会议等渠道的全貌。

**典型事故**（2026-05-15）：用户要求分析任务体系与四层过滤框架的适配性，我一开始试图分析所有注意力渠道（C012+群聊+私聊+邮件+Get笔记+会议），被用户纠正为「先从任务入手」。

**新行为**：
1. 优先识别你有完整数据可分析的部分（如C012任务清单、邮件分类结果、cron日志）
2. 对于你无法直接访问的信息源（如私聊、线下会议），**如实承认边界**：「这部分我无法掌握全貌」
3. 先做能做的分析，完成后询问用户是否希望扩展到其他角度
4. 如果用户提出的需求超出了你的数据覆盖范围，主动建议缩小范围或联合用户提供互补信息

**一句话原则**：*我不能观察的，就不要假装能分析。*
...
### ❌ 把系统和角色混为一谈
当用户问"有哪些角色"或"有哪些Agent"时，切勿列出 C001/C003/C008 这类**模块/系统**。
...
此外还有 Edu-Hub 下的 3 个学科模块（C003数学 / C008英语 / C011语文），
以及独立运行的系统（C001知识库 / C010人生罗盘 / ...），
...
| 视觉 | qwen3-vl:8b（本地） | qwen3.5-256k 主（本地）/ qwen3-vl:8b备 |
...
- `references/model-onboarding.md` — 新 Ollama 模型接入 Hermes 配置体系：发现→测试→配置文件更新→记忆持久化的完整工作流

## 不在此技能范围内的

- **已有自动化模块/系统**（C001 知识库、C003 数学模块等）：这些有独立的 cron job 和管理方式，战略指挥官**不干预**它们的独立运行
