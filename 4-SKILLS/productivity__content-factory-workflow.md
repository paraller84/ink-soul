...
add_manual_topic(
    title="AI Agent 记忆管理的三层架构实践",
    summary="从21.8K字符瘦身至3.6K：基于真实系统的记忆分层策略，含每轮注入降低83% token的实战数据。",
    topic_type="deep",
    category="工程实践"
)
"
...
也可以让用户在对话中确认后，由我直接执行添加。

### 与 Weekly Cron 的衔接

话题池中的每个条目带 `created_at` 时间戳，周一选题时 `content_pool.get_candidates()` 会按新鲜度+相关性评分。所以**不用等到了周一才想选题**——平时发现就保存，周一自然会被筛选出来。

核心变化：**选题从「周一集中想」变成「每天随手存」**。
