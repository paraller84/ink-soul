...
**Step 0-A: 前置预检（强制）** — 进入具体分析前必须执行的3项检查：

1. **❶ 全量前置清单检查**
   - 列出本次任务涉及的所有文件/页面/功能的**完整清单**（非分批）
   - 格式：编号 | 名称 | 操作类型（新增/修改/删除） | 状态（待确认/已确认）
   - 标记用户确认「无遗漏」后才进入下一步
   - 合并「先梳理清单」「先出页面清单」「全量前置」三层为统一入口

2. **❷ 需求收敛度检查**
   - 在启动前要求用户一次性提供当前批次的完整需求集合
   - 若用户表示「先做X再加Y」→ 要求先列完「当前批次」全部后再执行
   - 标记已追加的需求以评估对已完成工作的影响

3. **❸ 需求全貌审查（先观察再动手 升维版）**
   - 对数据/图片/设计/代码等输入，**先审视全貌再构思方案**，不跳过观察阶段
   - 适用范围不仅限于OCR——对所有「需要理解外部输入」的任务均强制
   - 典型场景：批量导入数据、参考现有系统重构、接入外部API
   - 禁止模式：只看第一项就开始设计 → 必须看完范围内全部输入
...
### 阶段 0: Step 4 进入门禁（⛔ 强制 — 任何 Web 任务）

> **2026-05-14 新增**：入阶自检。原因：B.5/B.6/C/D 已有完整定义，但在"快点交付"心态下多次被跳过（AI家庭教师系统即此案例），导致模板错误/按钮失效/OCR导入Bug交付后才暴露。此门禁强制我进入 Step 4 时先审视本次变更性质，确认对应验证阶段不可省略。

**在阶段 A 开始前，立即执行以下自检，并将结果记入 test_v1.json 的 `step4_gate` 字段：**
...
□ Flask 路由/模板/新页面/改 extends
  → 必须执行 B.6 模板渲染验证

□ HTML 交互/按钮/form action/JS 事件
  → 必须执行 B.5 交互按钮验证

|□ OCR/照片上传/文件导入/数据导入
  → 必须执行 D 黄金数据测试（测试数据: ~/.hermes/test-data/）

|□ 上述任意项勾选
  → 必须执行 E1 设计实现对照 + E2 用户动线测试

纯逻辑改动（无 UI）：至少运行 pytest 或脚本验证
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
结果: 涉及项=___, 对应阶段=___, 是否分配执行时间=是/否
```

**强制规则**：
1. 自检在阶段 A 之前完成，结果记入 test_v1.json
2. 标记的阶段**必须执行**，未执行即跳 Step 5 → 流程违规
3. "无UI变更"不豁免——至少运行 pytest 或功能脚本
4. 自检结果在 Step 4 结束时作为验证覆盖率凭证
...
### 阶段 B.5: 交互按钮/表单功能验证（🔥 强制 — Web 应用专属）

> 参考: `references/interactive-button-testing-patterns.md`

页面渲染通过后，立即执行交互功能验证：
1. 枚举所有 `<form>` 的 `action` 路由，确认每个都有对应的 POST handler
2. 对每个 POST handler 提交预期格式的参数，验证 HTTP 状态和 DB 变更
3. 检查模板中常见的 Flask/SQLite/Jinja2 陷阱（`.strftime()`、`float('inf')`、`action="#"`、隐藏字段不完整）
4. 验证状态变更操作（删除/审批/兑换）有幂等保护
5. 验证消费/兑换操作在创建记录前检查余额/权限
6. 测试数据库清理需 `PRAGMA foreign_keys = OFF` + `DELETE FROM sqlite_sequence`

**关键原则**：页面能渲染 ≠ 按钮能工作。本阶段和 C008 阶段是**独立的验证阶段**，不可省略。

### 阶段 B.6: Flask 模板运行时渲染验证（🔥 强制 — Web 应用专属）

> **核心洞察**：模板错误（Jinja2 变量不匹配、extends 链断裂、模板文件缺失、url_for BuildError）是 **运行时错误**，语法检查/静态扫描永远捕获不了。唯一可靠的方法就是**实际渲染每个模板**。
>
> **这就是「写完代码但用户一点就报 500/空白」的根因**。
>
> **所有涉及 Flask/Jinja2 模板的 Tier 1+ 任务，在声明「完成」之前，必须执行此验证。不执行此验证的交付物视为「未完成」。**

**验证方式**（三选一，优先用 test_client）：

```python
# 方式 A（推荐 — 最快最准）：Flask test_client 渲染每个路由
def verify_all_templates_render(app, skip_routes=None):
    \"\"\"遍历所有路由，渲染每个页面，捕获模板错误\"\"\"
    success = True
    skip = skip_routes or []
    
    for rule in app.url_map.iter_rules():
        url = rule.rule
        if url in skip or '<' in url:  # 跳过含参数的路由
            continue
...
```python
# 方式 B（对于需要 session 鉴权的路由）：模拟登录后访问
def verify_authenticated_routes(app, session_data, routes):
    with app.test_client() as c:
        ...
        for url in routes:
            ...
            assert len(resp.data) > 200, f'{url} 页面过短（可能空状态因变量名不匹配）'
            print(f'✅ {url} → {len(resp.data)} bytes')
```

```python
# 方式 C（兜底 — test_client 不支持的场景）：启动真实服务器 + curl
import subprocess, time, requests

proc = subprocess.Popen(['python3', 'app.py'], ...)
time.sleep(3)
for url in routes:
    r = requests.get(f'http://localhost:{PORT}{url}')
...
| 错误类型 | 错误信息 | 根源 |
|:---------|:---------|:-----|
| 变量未定义 | `jinja2.UndefinedError: 'knowledge_points' is undefined` | route 传 `points`，模板读 `knowledge_points` |
| 模板文件缺失 | `jinja2.TemplateNotFound: parent/analyze_result.html` | route 引用不存在的模板 |
| 端点找不到 | `werkzeug.routing.BuildError` | `url_for('wrong_endpoint')` 跨 Blueprint 引用 |
| extends 链断裂 | `jinja2.TemplateNotFound: base.html` | base.html 路径或名称错误 |
| block 不匹配 | `jinja2.TemplateSyntaxError: block name 'content' appears twice` | block 重复定义 |
| 参数类型错误 | `TypeError: 'NoneType' object is not iterable` | route 传 None 但模板用 `{% for %}` 遍历 |

**强制规则**：
1. **所有涉及 Web 页面的 Tier 1+ 任务**，在 Step 4 末尾（进入 Step 5 前）必须执行方式 A 或 B
2. **至少验证每条静态路由返回 200 + 页面有实质内容（>200 bytes）**
3. **含参数的路由**（如 `<student_id>`）提供测试参数，或手动构造测试 URL
4. **发现的每个模板错误必须修复**并重新验证，不得「标注为已知问题」进入 Step 5
5. **验证结果**记录在 test_v1.json 的 `result.template_rendering` 字段：
   ```json
   {
     "template_rendering": {
       "method": "test_client",
...
同时，质量门禁扫描后，**必须**执行阶段 B.5（Flask 模板运行时渲染验证）——质量门禁的静态扫描无法捕获模板渲染错误，两个验证互为补充，**缺一不可**。
...
### 阶段 D: 黄金数据测试（🔥 强制 — 涉及 OCR/拍照/导入功能专属）

> **根因**：OCR 识别、拍照导入、文件上传功能高度依赖实际数据质量。仅靠模板渲染或点击测试无法发现
> 字符识别残缺→结构化解析失败→数据库字段不完整→前端展示空白的问题链路。
> **只有用真实用户数据跑完整链路，才能捕获这些错误。**
>
> **参考**：`~/.hermes/test-data/ai-home-tutor/MANIFEST.md`
> （含 10 份真实样例：数学试卷×2、语文试卷×3、英语试卷×2+单词×1、会写字×1、会认字×1）

**执行流程**（以语文拍照导入为例，各系统按实际修改）：

```bash
# D1: OCR 识别验证 — 真实图片能否被正确识别
python3 -c "
from ocr_engine import OCRProcessor
import os
TEST_DATA = os.path.expanduser('~/.hermes/test-data/ai-home-tutor')
ocr = OCRProcessor()
for img in os.listdir(TEST_DATA):
    if not img.endswith('.jpg'): continue
    path = os.path.join(TEST_DATA, img)
    result = ocr.recognize(path)
    assert result and len(result.get('text','')) > 50, \
        f'❌ OCR 识别不完整: {img} → {len(result.get(\"text\",\"\"))} chars'
    print(f'✅ OCR: {img} → {len(result[\"text\"])} chars')
"

# D2: 结构化解析验证 — 识别结果能否正确结构化
python3 -c "
# 根据实际系统修改导入函数和断言逻辑
from import_pipeline import parse_content
import os
TEST_DATA = os.path.expanduser('~/.hermes/test-data/ai-home-tutor')
for img in os.listdir(TEST_DATA):
    path = os.path.join(TEST_DATA, img)
    parsed = parse_content(path)
    assert 'chars' in parsed or 'questions' in parsed, \
        f'❌ 解析结果缺少内容字段: {img}'
    print(f'✅ Parse: {img} → {len(parsed.get(\"chars\", parsed.get(\"questions\", [])))} items')
"

# D3: 入库验证 — 结构化数据能否正确写入数据库
python3 -c "
from db import get_db
db = get_db()
rows = db.execute('SELECT COUNT(*) FROM chars').fetchone()[0]
assert rows > 0, '❌ 数据库无数据'
print(f'✅ DB: {rows} 条记录写入')
"

# D4: 前端展示验证 — 数据能否在前端正确渲染
python3 << 'EOF'
from app import app
with app.test_client() as c:
    with c.session_transaction() as sess:
        sess['student_id'] = 'test'
    resp = c.get('/chinese/chars')
    assert resp.status_code == 200, f'❌ 列表页: {resp.status_code}'
    assert len(resp.data) > 500, '❌ 列表页内容过短（可能无数据渲染）'
    print(f'✅ 前端展示: 200 → {len(resp.data)} bytes')
EOF
```

**硬性规则**：
1. D1-D4 任一阶段失败 → 不可进入 Step 5，返回 Step 3 修复
2. 新增上传/OCR 功能时，**必须要求用户提供至少 1 份真实样例**（或使用已有黄金数据）
3. 测试结果记录在 test_v1.json 的 `result.golden_data_test` 字段
...
```json
{
  "golden_data_test": {
    ...
    "frontend_pass": true,
    "samples": [
      "img_7a85bbecea85.jpg (语文试卷)",
      "img_9d826e251228.jpg (会写字)",
      "img_c1d49f91b5ea.jpg (会认字)",
      "img_5a06bb366b06.jpg (英语单词/词组/短句)"
    ]
  }
}
```
...
### 阶段 E: 设计实现对照 + 用户动线测试（🔥 强制 — 任何 Web 应用）

> **根因**：功能实现了但和设计不一致、页面独立渲染正常但用户沿着导航走几步就断链——这两种问题
> 只有"逐行对照设计文档"和"模拟真实用户操作链路"才能发现。
>
> 参考：Step 0 交付物 `req_analysis_v1.json`（需求清单）、Step 2 交付物 `design_v1.json`（页面动线图）

**E1: 设计实现对照（逐项打勾）**

从设计交付物中提取**功能清单**，逐项对照代码中是否实现：

```python
# 伪代码逻辑
design_features = [
    {"id": "F01", "desc": "学生门户页展示今日任务", "page": "/portal"},
    {"id": "F02", "desc": "今日任务显示10道练习题", "page": "/practice"},
    {"id": "F03", "desc": "答题后显示对错和解析", "page": "/practice/result"},
    ...
]

for feature in design_features:
    # 检查对应路由是否存在
    route_exists = any(feature["page"] in rule.rule for rule in app.url_map.iter_rules())
    # 检查页面内容是否包含预期元素
    with app.test_client() as c:
        resp = c.get(feature["page"])
        content_ok = feature_keyword_in_response(feature, resp.data)
    
    assert route_exists and content_ok, f"❌ {feature['id']}: {feature['desc']} 未实现"
    print(f"✅ {feature['id']}: {feature['desc']}")
```

**E2: 用户动线测试（模拟完成一条完整用户路径）**

从设计文档的**动线图**（页面流）中提取典型用户路径，用 browser 或 test_client 模拟完整操作链：

```
典型动线示例（以教育系统学生为例）：
  门户页 → 点击"今日练习" → 答题页 → 逐题作答提交 → 结果页 → 点击"查看错题" → 错题本 → 点击"返回首页"

测试方法：
  1. 访问门户页 /portal  →  ✅ 渲染
  2. 点击"今日练习"链接 → 跳转到 /practice → ✅ 渲染
  3. 提交答案表单 → POST /api/answer → ✅ 200 + 数据库记录
  4. 跳转结果页 /practice/result/1 → ✅ 渲染且显示分数
  5. 点击"错题本"链接 → 跳转到 /wrong-questions → ✅ 渲染且显示错题
  6. 点击"返回首页" → 回到 /portal → ✅ 完成闭环
```

```bash
# 动线测试脚本模板（用 test_client）
python3 << 'EOF'
from app import app
...
test_flows = [
    {
        "name": "学生练习完整流程",
        "steps": [
            {"method": "GET", "url": "/portal", "expect": 200, "check": ["今日任务", "专项练习"]},
            {"method": "CLICK_AND_GET", "from_url": "/portal", "link_text": "今日练习", "expect": 200, "check": ["题目"]},
            {"method": "POST", "url": "/api/answer", "data": {"question_id": 1, "answer": "A"}, "expect": 200},
            {"method": "GET", "url": "/practice/result/1", "expect": 200, "check": ["得分", "解析"]},
            {"method": "GET", "url": "/wrong-questions", "expect": 200, "check": ["错题"]},
        ]
    },
    {
        "name": "家长后台查看流程",
        "steps": [
            {"method": "GET", "url": "/parent/dashboard", "expect": 200, "check": ["学习报告"]},
            {"method": "GET", "url": "/parent/children", "expect": 200, "check": ["子女"]},
        ]
    }
]
    ...
    for flow in test_flows:
        ...
        for step in flow["steps"]:
            if step["method"] == "GET":
                ...
                resp = c.post(step["url"], data=step.get("data", {}))
            
            assert resp.status_code == step["expect"], \
                f"❌ {step['url']} → {resp.status_code} (期望 {step['expect']})"
            
            content = resp.data.decode('utf-8', errors='replace')
            for keyword in step.get("check", []):
                assert keyword in content, \
                    f"❌ {step['url']} 缺少关键词: {keyword}"
            
            print(f"  ✅ {step['method']} {step['url']} → {step['expect']}")
        print(f"  ✅ 动线通过")
EOF
```

**硬性规则**：
1. **每一条动线必须完整模拟**——从入口到出口不可跳过中间步骤
2. **动线提取自 Step 2 设计交付物**的页面动线图/页面流图
3. E2 失败 → 不可进入 Step 5，修复动线中的断链后重测
4. 测试结果记录在 test_v1.json 的 `result.design_compliance` 和 `result.user_journey` 字段
...
Flask+SQLite 应用的端到端测试有多个特有陷阱（autoincrement、strftime、session隔离等），详见 `references/flask-e2e-testing-patterns.md`。
...
象限 C: 集成测试 (IT-HTML) — 页面渲染验证
  ...
  ├── 🔥 **模板运行时渲染验证** — 启动 Flask test_client，访问每个路由捕获：
  │   ├── Jinja2 Variable undefined（routes 传参名 vs 模板变量名不匹配）
  │   ├── Jinja2 TemplateNotFound（route 引用不存在的模板文件）
  │   ├── url_for BuildError（跨 Blueprint 引用错误端点）
  │   ├── TemplateSyntaxError（block 结构损坏）
  │   └── 页面过短（<200 bytes → 静默空渲染）
  └── **所有静态路由模板渲染 200 才算页面渲染验证通过**
...
0. **🔥 最致命的陷阱 — 模板错误是运行时错误，语法检查永远抓不住**。
   这意味着：写完代码后只做 `compile()` 语法检查 = **没有验证模板是否正确**。一个模板可能语法完美但路由传的变量名错了、base.html 的 block 结构改了、跨 Blueprint 的 url_for 指错了端点。
   
   **根因**：Jinja2 模板在渲染时才解析变量引用和 url_for。`compile(html)` 只检查 Python 语法，对 Jinja2 标签一无所知。唯一的验证方式是**实际渲染**。
   
   **这正是用户痛点「刚说完成，一点进去就发现模板错误」的根源**。解决方案：必须执行阶段 B.5（Flask 模板运行时渲染验证）。
...
## 新增陷阱 21：patch 工具不验证 Jinja2 模板语法（静默损坏陷阱）

**发现场景**（2026-05-14 英语门户改造）：使用 `patch` 工具重写模板 JS 代码块后，英语门户 portal 页面返回 500。检查发现 Jinja2 模板末尾出现了两个 `{% endblock %}`，一个来自于 old_string 覆盖区域外的残留，另一个来自于 new_string 的尾部。patch 工具不验证模板语法，静默写入了损坏文件。

**根因**：`patch` 工具的字符串匹配是基于文本的，对 Jinja2 语法一无所知。当 old_string 和 new_string 包含相同的 Jinja2 标签（如 `{% endblock %}`），且替换边界触及标签所在行时，可能产生：

- **重复的标签**：`{% endblock %}\n{% endblock %}`（两个 endblock 关闭同一个 block）
- **标签不匹配**：`{% if %}` 打开但 `{% endif %}` 未关闭
- **block 截断**：一个 block 的内容被截断混入另一个 block

```html
<!-- ❌ patch 后的损坏内容 -->
</script>
{% endblock %}
{% endblock %}    ← 多余的 endblock！Jinja2 会报错
```

**为什么 Jinja2 不优雅降级**：Jinja2 对未关闭的标签和多余的关闭标签都会直接报 `TemplateSyntaxError` 500 错误，不是静默忽略。但 patch 工具无法检测这一点——它只检查文件是否被写入。

**预防**：每次对 `.html` 模板文件 patch 后，运行 Jinja2 语法检查：
...
app = Flask(__name__)
env = Environment(loader=FileSystemLoader(['english/v3/templates', 'templates']))
try:
    env.get_template('v3/portal_v4.html')  # 替换为实际模板路径
    print('Jinja2 template: ✅')
except Exception as e:
    print(f'Jinja2 template: ❌ {e}')
"
```

**排查标志**：patch 后 HTML 页面返回 500，服务器日志包含 `jinja2.exceptions.TemplateSyntaxError`，错误行号靠近模板文件末尾。

## 新增陷阱 22：CSS 文件多次 patch 导致重复定义/结构损坏
...
## 新增陷阱 35：Flask `url_for` 跨 Blueprint 引用导致 500（BuildError 无声陷阱）

**发现场景**（2026-05-13 Edu-Hub 中文模块门户统一）：打开中文门户 `/chinese/v3/portal` 返回 500。Flask 日志显示 `BuildError: Could not build url for endpoint 'chinese_v3_bp.parent_wrong_book'`。

**根因**：模板中的 `url_for('chinese_v3_bp.parent_wrong_book')` 引用的 endpoint 属于**另一个 Blueprint**（`chinese_v3_parent_bp`）。Flask 的 `url_for()` 在模板渲染时抛出 `BuildError`，不是 404 — 是**500 内部错误**。

```jinja
{# ❌ 错误：endpoint 不在当前 Blueprint 上 #}
<a href="{{ url_for('chinese_v3_bp.parent_wrong_book') }}">
{# chinese_v3_bp 没有 parent_wrong_book 路由，它在 chinese_v3_parent_bp 上 #}

{# ✅ 正确选项 A：使用硬编码路径（推荐跨 Blueprint 时） #}
<a href="/chinese/v3/parent/wrong-book">

{# ✅ 正确选项 B：确认 endpoint 名并使用正确 Blueprint 前缀 #}
{# 但需确保 target Blueprint 已注册 #}
```

**为什么不是明显的编译错误**：Jinja2 不会预编译 `url_for()` 调用 — 它只在实际渲染时**运行时**调用 Flask 的 URL 构建器。这行代码看起来合法（语法正确、变量名存在），只有在渲染到这一行时才会抛出异常。

**区别于普通 404**：
- 路由不存在 → 返回 404（HTTP 级别）
- url_for 端点不存在 → 返回 500（模板渲染级别）

**排查标志**：
- 打开页面直接 500
- 没有明显的 Python 代码语法错误
- 服务器日志包含 `BuildError` 关键词
- 错误消息中显示 `endpoint` 名称

**排查步骤**：
1. 立即查看 Flask 服务器日志寻找 `BuildError` 或 `werkzeug.routing.exceptions.BuildError`
2. 确认问题 endpoint 属于哪个 Blueprint（检查所有 `@blueprint_name.route()` 定义）
3. 确认模板中使用的 Blueprint 名称与实际定义的 Blueprint 名称一致（注意命名：`chinese_v3_bp` vs `chinese_v3_parent_bp`）

**预防**：
1. 在 Step 2.5 骨架搭建阶段，对模板中每个 `url_for()` 调用做「端点存在性验证」——搜索 `@blueprint_name.route()` 确认端点确实存在
2. 特别注意跨 Blueprint 引用：父模板中的链接可能指向不同 Blueprint 的路由
3. 如果链接目标在另一个 Blueprint 上，使用硬编码路径（`/path/to/page`）而非 `url_for()`，除非能确认 `build` 可用
4. 推荐模式：**同 Blueprint 内部用 `url_for()`，跨 Blueprint 用硬编码路径**

## 🔴 新增红线：命名空间/Blueprint 级重构必须走完整 C005 流程

**发现场景**（2026-05-14）：统一三个教育模块的 Blueprint 名（`math_v2`→`math_bp`、`eng_v3`→`english_student_bp`、`chinese_v3_bp`→`chinese_bp`）和 URL 前缀（语文 `/chinese/v3/`→`/chinese/`）时，直接使用 3 个并行子代理执行，**未走 C005 流程**。用户批评：「我跳过 C005 直接执行」。

**为什么这是红线**：
- 涉及 **3 个模块 × 15+ 文件 × 100+ 引用**，影响面不亚于数据库迁移
- 并行子代理无法感知彼此的工作 → `app.py` 中的 Chinese 注册（`chinese_v3_bp`→`chinese_bp`）被漏掉
- 重启前无验证步骤 → 未发现漏改，用户访问时直接 404/500

**执行铁律**：

1. **命名空间/Blueprint 级重构默认 Tier 3**（全流程 C005），无论看起来多「简单」
   - 即使只有 find-and-replace 级别的改动（正如本次），影响链跨越多个独立模块，需要集中协调
2. C005 Step 0（需求分析）必须输出**完整文件清单 + 依赖拓扑**
   ```json
   {
     "step_id": "req_analysis_v1",
     "result": {
       "affects_cross_module_communication": true,
       "files_to_modify": [
         {"path": "app.py", "changes": ["blueprint import", "blueprint registration", "subject links", "startup print"]},
         {"path": "english/v3/routes.py", "changes": ["59× eng_v3 → english_student_bp"]},
         {"path": "chinese_v3/__init__.py", "changes": ["blueprint name"]},
         ...
       ],
       "coordination_points": ["app.py 必须最后修改（等所有模块的 export 名确定）"]
     }
   }
   ```
3. **必须最后修改 app.py** — 等所有模块的 `__init__.py` 改名确认后，再一次性更新 app.py 中的 import + registration + subject selector links + startup print
4. **重启前必须执行完整性扫描**：
   ```bash
   # 检查残留旧引用
   grep -r "old_blueprint_name" --include="*.py" --include="*.html" edu-hub/ | grep -v artifacts | grep -v __pycache__
   grep -r "old_url_prefix" --include="*.py" --include="*.html" edu-hub/ | grep -v artifacts | grep -v __pycache__
   ```
5. **外部引用检查** — 如果变更了 URL 前缀，必须检查以下位置同步更新：
   - `app.py` 中的科目选择链接（`_render_subject_select()` 内联 HTML 函数）
   - `app.py` 中的启动信息打印（`print(f"  /chinese/ 语文学生端")`）
   - Windows 端口转发规则（`netsh interface portproxy` 中的 URL）
   - 飞书文档中的入口链接（如果有）

**判别器** — 以下情况自动标记为「Tier 3 跨模块重构」，强制 C005 全流程：

```python
def is_cross_module_refactoring(changes: list) -> bool:
    """判断是否需要 Tier 3 C005 流程"""
    signals = []
    # 涉及 2+ 个独立 Blueprint
    blueprints = set(c.get("blueprint") for c in changes if "blueprint" in c)
    if len(blueprints) >= 2:
        signals.append("多 Blueprint 变更")
    # 涉及 app.py 注册
    if any("app.py" in c.get("file", "") for c in changes):
        signals.append("app.py 注册变更")
    # 涉及 URL 前缀变更
    if any("url_prefix" in c for c in changes):
        signals.append("URL 前缀变更")
    # 涉及 10+ 文件
    if len(changes) >= 10:
        signals.append("≥10 文件变更")
    return len(signals) >= 2
```

## 新增陷阱 36：设计文档版本混淆（多版本同存时的陷阱）

**发现场景**（2026-05-14 AI家庭教师 v2.2 实施）：我误读了v2.0（27页）而非最新的v2.2（34页），向用户汇报了错误的页面数量。用户纠正后才发现缓存目录中有8个版本的HTML文件（v1.2→v2.2）。

**根因**：设计迭代过程中，多个版本的设计文档HTML文件同时存在于缓存目录中。文件名仅差版本号后缀（`v1.9.html` vs `v2.0.html` vs `v2.2.html`），容易误读旧版本。文件名中的版本号（如"v1.9"→"v2.2"）增量变化，阅读者可能潜意识里认为"最新的是某个版本"但实际不是。

**排查方法**：
```bash
# 找到目标目录下所有版本文件，按修改时间排序
ls -lt ~/.hermes/cache/ai-home-tutor-design-*.html | head -5

# 检查文件标题行确认版本
head -5 ~/.hermes/cache/ai-home-tutor-design-v2.2.html | grep -oP 'v\d+\.\d+.*?\d+页面'
# 输出示例: v2.2 完整版 · 34页面
```

**预防清单**（启动实施前必做）：
1. **用 `find` + `sort` 确认存在多少版本**
   ```bash
   find ~/.hermes/cache -name '*设计*' -o -name '*tutor*v*' | sort
   ```
2. **读取文件标题行** — 设计文件的 `<title>` 或 `<h1>` 通常有版本号和页面数
   ```bash
   head -5 <file> | grep -E '(version|v\d|页面|page)'
   ```
3. **与记忆中的版本号交叉比对** — 记忆中的版本号（如"v1.9设计完成"）可能不是最新
4. **如发现多个版本，用 `ls -lt` 按修改时间排序确认最新**
5. **读取页面索引表**确认总页面数，而非依靠文件名或目录结构的直觉判断

## 新增陷阱 36：科目选择器入口链接与 portal 路径不同步

**发现场景**（2026-05-13 Edu-Hub 门户统一后）：用户从 `/student/` 科目选择页点击「英语」和「语文」卡片，进入的页面与设计方案不一致。

**根因**：`app.py` 中 `_render_subject_select()` 硬编码了科目入口链接（`eng_link = "/english/student/"`、`chi_link = "/chinese/v3/"`）。这些路径指向的是模块的**根路由**（`index()`），而非**实际的门户页面**（`portal()`）。根路由通过 302 跳转到 portal，但用户的浏览器收到了跳转而非直接渲染。

```python
# ❌ 旧：走 302 → 门户（额外 HTTP 跳转，如果 portal 路由要求 session 则可能失败）
math_link = "/math/student/"
eng_link = "/english/student/"    # → 302 → /english/student/portal
chi_link = "/chinese/v3/"         # → 302 → /chinese/v3/portal

# ✅ 新：直接指向门户（一步直达）
math_link = "/math/student/"      # 已直连，不变
eng_link = "/english/student/portal"
chi_link = "/chinese/v3/portal"
```

**为什么是陷阱**：
1. 走 302 跳转后，如果目标路由有 `@require_role` 装饰器，session 状态可能被清空或检查失败
2. 模板 developers 修改 portal 路由路径后，容易忘记同步更新 `app.py` 中的科目选择链接
3. `app.py` 中的 `_render_subject_select()` 是一个**内联 HTML 函数**（非 Jinja2 模板），不在常规的模板目录中，容易被遗漏

**排查标志**：
- 从 `/student/` 点击科目后进入的页面与预期的 portal 设计不一致
- 科目入口实际渲染的是另一个页面（如模块的 index 页面而非 portal 页面）
- 浏览器地址栏的 URL 与预期的 portal URL 不一致

**预防**：
1. 每次修改模块 portal 路径后，立即检查 `app.py` 中的 `_render_subject_select()` 链接是否同步更新
2. 用 `curl -L` 确认每个入口链接是否直达最终页面（200 OK）：
   ```bash
   for path in "/math/student/" "/english/student/portal" "/chinese/v3/portal"; do
     echo -n "$path: "
     curl -s -L -o /dev/null -w "%{http_code}\n" http://localhost:5002$path
   done
   ```
3. 把 `_render_subject_select()` 函数中定义的链接视为「配置项」——模块门户路径变更时这些链接必须跟着变
...
**发现场景**（2026-05-14 AI家庭教师构建）：使用 Hermes Agent venv 创建新 Flask 应用时，假设 `flask_sqlalchemy` 可用，但 Hermes Agent venv 仅安装了 `Flask` 和 `Werkzeug`，未安装 `flask_sqlalchemy`。导致整个数据层（models + services + routes）完成后才发现无法导入，需全部重写为原生 `sqlite3`。

**全量重写的代价**：7个services + 4个route文件 + app.py → 36小时token消耗，约2.8M input tokens。

**预防**：在 Step 0 末尾（需求确认后、编码开始前），执行依赖验证：

```python
# 在 Step 0 末尾执行依赖预检
from hermes_tools import terminal

def verify_dependencies(required_packages):
    \"\"\"检查目标 Python 环境是否包含所需包\"\"\"\n    
    for pkg in required_packages:
        cmd = f'python3 -c \"import {pkg}\" 2>&1'
        result = terminal(cmd)
        if 'ModuleNotFoundError' in result['output']:
            raise ImportError(f'❌ 缺少依赖: {pkg}。可用 pip install {pkg} 安装，或改用原生实现')

# 在 Step 0 输出后调用
verify_dependencies(['flask_sqlalchemy'])  # 检查 ORM
```

**回退方案**：当 `flask_sqlalchemy` 不可用时，直接使用原生 `sqlite3` + `db.py` 工具模式：
- `from db import get_db, dict_from_row, dicts_from_rows`
- `db = get_db(); db.execute(sql, params).fetchone()/.fetchall()`
- `db.commit()` 替代 `db.session.commit()`
- 所有 WHERE 条件用 `?` 占位符传递参数

此模式已在 AI家庭教师项目中验证可行，15个Python文件全部从SQLAlchemy迁移到原生sqlite3。
...
原则6：写完必验证（含运行时渲染验证）
  案例：B1 patch() 静默失败、模板渲染错误无声无息
  习惯化：工具默认执行 compile() 语法检查
          replace_all=True 在 .py 上被直接拒绝
          Flask 模板必须用 test_client 实际渲染验证
          不渲染 = 不算写完
...
## 新增陷阱 37：从设计规格直接按页面索引数页面（遗漏中间路由页面）

**发现场景**（2026-05-14 AI家庭教师系统）：设计文档第0节索引表列出18个页面，我直接按18页开始做架构。用户指出实际应有34+页面——遗漏了学生身份选择、科目选择、练习模式选择、错题重做、积分兑换确认等所有**中间过渡路由页面**。

**根因**：设计文档的页面索引表通常只列出**核心功能页面**（有内容原型/线框图的页面），不会列出所有路由中间节点。从索引表数页面会始终低估实际页面数量。

**正确的做法：动线审计法（Flow Trace Audit）**——在Step 0需求分析中，强制输出一张「完整页面动线图」：

```
如何产出页面动线图：
1. 从系统入口开始，走完整条用户动线
2. 每个决策点（角色选择→科目选择→模式选择→进入功能→反馈→返回）都是一个路由节点
3. 标注每个节点的角色（公共/学生/家长）
4. 标注每个节点的←返回路径
5. 标注每个节点的数据来源（是否需要API调用）
6. 对比设计文档索引表，找出「索引表未列但动线需要」的页面
7. 最终页面数 = 索引表页面 + 动线推导页面
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
Step 0交付物中增加 `page_inventory` 字段记录动线审计结果。

**排查标志**：
- 设计文档页面数少（<20），但动线图显示有多个分支路径
- 核心页面之间有明显的「选择」步骤（如选科目、选模式、选学生）
- 弹窗/确认页在索引表中未被独立统计

**预防**：将动线审计作为 Step 0 的强制输出项，列入 req_analysis_v1.json 的 `result.page_inventory` 字段。

## 新增陷阱 38：设计文档未明确架构归属时默认错误

**发现场景**（2026-05-14 AI家庭教师系统）：设计文档描述了一个独立的教育管理系统（有自己的入口、PIN验证、家长中心），但我默认将其归为「Edu-Hub下的模块」，直到用户纠正：「这是一个完全彻底的独立系统」。

**根因**：对设计文档的架构归属做了「最小改动假设」——因为Edu-Hub已是运行中的教育平台，就默认新系统也应接入它。架构归属应基于设计文档本身的描述来判断。

**判别规则**：
```
独立系统特征（任意一条满足即倾向独立）：
  └─ 有独立的系统入口/欢迎页（不是已有应用的子页面）
  └─ 有独立的用户认证流程（PIN码、独立注册）
  └─ 有自己的数据库模型（与现有系统不共享的数据结构）
  └─ 有独立的前端框架规范（视觉风格与现有系统不同）
  └─ 文档中未出现「接入」「集成」「作为XX的补充」等描述

子系统/模块特征（需明确证据）：
  └─ 文档说明「作为XX系统的扩展」「在XX基础上」
  └─ 共享用户体系（复用现有session/认证）
  └─ 共享数据库表或数据模型
  └─ 文档中明确列出依赖的系统文档或API

当设计文档未明确说明时，按「独立系统」假设开始架构设计。
```

**纠正代价**：错误假设→提交Step 0给用户→被纠正→重新做架构→浪费1-2轮对话。

**预防**：
1. 拿到设计文档后，先对照上述判别规则做架构归属判断
2. Step 0 交付物中明确输出「架构归属：独立系统」并标注判断依据
3. 如果判断依据不充分（文档描述模糊），在 Step 0 中列出「不确定项，需用户确认」
4. 核心原则：**当不确定时，默认独立系统**（独立系统可以后续拆为模块，模块难以后期独立）
