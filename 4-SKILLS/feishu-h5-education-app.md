...
家长端登录使用 PIN 码（默认 123456）而非密码。页面采用**台式 PIN 输入面板**风格：
- 6 位数字 PIN，通过虚拟数字键盘输入
- 输入时以圆点（`.pin-dot.filled`）反馈，不显示数字明文
- 输入满 6 位自动提交
- 错误 5 次后锁定
- 模板位置：`templates/parent/pin_verify.html`
...
<div class="pin-dots" id="pinDots">
  <div class="pin-dot filled" data-index="0"></div>  <!-- 已输入 -->
  <div class="pin-dot" data-index="1"></div>          <!-- 未输入 -->
  ...
</div>
...
| 状态 | 样式 |
|:----|:-----|
| 未输入（空圈） | 16px 圆，白色背景，灰色边框 (`#ccc`) |
| 已输入（实心） | 16px 圆，紫色填充 (`#6c63ff`)，紫色边框 |
| 聚焦态 | 紫色边框 + `box-shadow: 0 0 0 3px rgba(108, 99, 255, 0.15)` |

### 双视图渲染

模板同时渲染**手机视图（375px）**和**平板视图（600px）**，使用独立 DOM 副本和 JS 函数（`pressKey()`/`pressKeyPad()`），共享 `pinValue` 状态变量。未来移除双帧预览模式后应合并为一套逻辑。

### 设计决策追踪

**⚠️ PIN 显示方式属于交互设计决策，必须在原型/设计文档中明确定义，并与代码实现保持同步。** 当用户提出 PIN 显示方式修改要求时（如从圆点改为短暂显示数字、或从明文改为星号等），必须记录决策到以下至少一处：
1. 原型 HTML 文件（prototypes/ 或 pad-prototypes/ 目录）
2. 设计方案文档（上传至飞书云盘对应文件夹）
3. 本技能的相关节段

**在修改实现前**，必须先在历史会话中搜索（`session_search`）或在设计文档中查找（`web_extract`/飞书 API）定位之前的设计决策，确认要改成的确切方式。不可以假设或猜测用户的意图。

### 相关陷阱

- **双视图同步**：手机和 pad 视图的 JS 函数独立（`pressKey` vs `pressKeyPad`），修改一个时必须同步修改另一个
- **自动提交**：输入满 6 位自动 submit，但模板含 `for i in range(6)` 硬编码 6 位——改为可变长度时需同步改
- **hidden input + JS 状态**：pin 值同时保存在 JS 变量 `pinValue` 和 hidden input 的 `value` 中，两者必须一致

#### 用户报告"圆点不变化"排查流程

当用户反馈"输入密码后圆点没变化"时，按以下顺序排查：

1. **检查 DOM 是否存在** — `document.querySelectorAll('.pin-dot').length`
2. **检查 JS 是否工作** — 点击数字后查 `pinValue` 是否更新
3. **检查 class 是否切换** — `document.querySelector('.pin-dot').className` 应含 `filled`
4. **检查计算样式** — `getComputedStyle(dot).backgroundColor` 应为 `rgb(108, 99, 255)`

如果以上全部正常但用户仍说没变化 → **浏览器缓存**。让用户按 **Ctrl+F5**（强制刷新）或打开 DevTools → 网络 → 勾选"禁用缓存"后刷新。

> ⚠️ 陷阱：不要轻易怀疑 JS/CSS 代码。服务端验证通过的代码在用户端不生效，缓存问题的概率远高于代码 bug。先用浏览器验证再动代码。
