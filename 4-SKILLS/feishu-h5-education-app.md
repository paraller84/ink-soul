...
| 步骤 | 操作 | 说明 |
|:----|:-----|:------|
| ① | 备份旧 v3/ | `cp -a v3/ v3_backup_$(date +%Y%m%d)` |
| ② | 删除旧模块 | `rm -f range_selector.py question_engine.py learning_session.py` |
| ③ | 创建新模块 | `data_manager.py + dictation_engine.py + practice_engine.py` |
| ④ | 重写 routes.py | 替换为 3 页面 + 12+ API 端点 |
| ⑤ | 重写模板 | 统一门户 + 听默/练习双模块 |
| ⑥ | app.py | `redirect("/v3/portal")` 替代 `redirect("/v2/")` |
| ⑦ | v2 保持不变 | 管理端完全不受影响 |
...
8. **CSS 冲突**：v2 的 CSS 通过 `/v2/static/style.css` 独立加载，避免污染 v1 样式
