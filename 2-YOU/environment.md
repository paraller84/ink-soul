...
§
飞书上传经验（2026-05-17）: upload_all 需要正确的 multipart form-data 构建（boundary uuid + 5个字段：file_name/parent_type/parent_node/file）。文件夹 Token 已知存在漂移问题——所有已注册的子文件夹Token均返回404（系统文档LQx1fl0v8lB1XFdAI29czWeznVh, 能力体系HA85fFkMQlwPijdIBxccQDQQnxf, Hermes生成Otppfr9EelPIawdezL2csUXCnoh）。需要在手动调用 list_folder API 获取最新 Token 后再上传。
§
