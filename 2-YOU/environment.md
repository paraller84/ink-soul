...
§
C012飞书待办正在迁移至滴答清单(dida365.com)。2026-05-20方案已出，待用户提供TickTick开发者应用Client ID+Secret后执行。迁移后管线：C013 Get笔记→我去重归类+优先级→ticktick-sdk(Python)→滴答清单。晨间简报届时改为从滴答清单拉取。feishu-task-management-system skill已更新v5.0含迁移公告。
§
...
§
飞书云盘三区生命周期管理 v2.0 (2026-05-20上线): 每个项目/板块目录下增加 _archive/(历史归档只读保留) 和 _drafts/(草稿超7天自动清理) 子目录。命名规范强制含状态标签(FINAL/WIP/DRAFT/OBSOLETE)+日期。生成纪律：旧版FINAL上传前先移_archive；DRAFT直接入_drafts。每周一06:00自动审计(cron 3d28740068d2)。Tokens已注册到feishu-drive-directory-standard skill。
§
滴答清单迁移使用 dida365.com（中国版），非 ticktick.com。开发者后台在 developer.dida365.com/manage。OAuth2 授权时必须设 TICKTICK_HOST=dida365.com。ticktick-sdk v0.4.3 安装于 hermes venv。access_token 已获取（ff525484...，有效期至2026-11-17）。SDK 需双API认证（V1 OAuth2 + V2 用户名密码），V2 凭证未提供。
§
=== 滴答清单迁移 (2026-05-21) ===
C012 飞书待办迁移至滴答清单(dida365.com)已完成 OAuth2 授权。凭证：TICKTICK_CLIENT_ID=NStHBloR46E5R9yJ7f, TICKTICK_ACCESS_TOKEN=ff525484-d789-4b6f-82ac-7833462c7845 (180天有效), TICKTICK_HOST=dida365.com。默认项目「待执行清单」(5f4c318e785f11079c5a803c)。桥接脚本 ~/.hermes/scripts/ticktick-bridge.py（直连 V1 API，不依赖V2会话认证）。迁移脚本 ~/.hermes/scripts/c012-to-dida-migrate.py。当前第1-3天并行期：新任务写入滴答清单，C012飞书Task只读不新建。账号 paraller84@163.com。开发者后台: https://developer.dida365.com/manage。redirect_uri 已配置为 http://127.0.0.1:8080/callback。
§
滴答清单能力匹配方案：时段-精力匹配（上午高峰决策/下午平稳执行）、项目定义5条门禁（明确目标+≥3子任务+多部门+时间线+跨日常流）、委派标签体系（委·照宇=EAST/客户数据/数据中台，委·庄经纬=知识管理，委·张子越=外部数据/资产管理，委·曹爽=数据安全，委·科创中心）、突发事件缓冲槽（📥待归位列→09:00归类排期）、通勤标签库（通勤·读/回/整理→17:00自动推清单）。完整方案在feishu-task-management-system skill的references/ticktick-task-energy-strategy-v2.html。
§
=== 滴答清单迁移执行状态(2026-05-21) ===
✅ OAuth2 + ticktick-sdk v0.4.3 + 桥接脚本 + 3个项目(EAST治理/委派跟进/会议待办) + 晨间全景脚本
⏳ V2标签需用户提供滴答密码(V1不够)；C012 ~21条活跃任务未迁移；晨间全景改投办公室群需chat_id
§
滴答清单项目结构已建立(2026-05-21): ⚡EAST治理(6a0e5b5ee4b0e87193184017), 📬委派跟进(6a0e5b5ee4b0e19c53ff35d4), 📋会议待办(6a0e5b5ee4b04e66f3f843fe)。21条C012活跃任务已全部迁移到滴答清单。晨间全景cron(8114210bdc6c)改为no_agent脚本模式(morning-panorama-ticktick.py)，工作日09:00投递🗓️办公室群。ticktick-sdk v0.4.3已安装。V2 API标签创建因access_forbidden未成功，但V1 API创建任务时tags字段可用。
§
email-to-knowledge-pipeline skill 新增 references/historical-attachment-extraction.md — 从历史 .eml 附件提取文档内容（7z/PDF/xlsx全流程）。适用场景：需查找自动导出系统部署前（~2026-05前）的历史邮件附件。关键发现：历史邮件常存在于 Fw_ 转发链邮件的附件中。
§
滴答V1 API只写不读（所有GET端点500/404）。已建立双轨方案：①本地注册表(~/.hermes/data/ticktick-registry.json)存储任务状态+分类 ②ticktick-sync.py脚本通过浏览器IndexedDB同步实际数据。密码已提供paraller84@163.com，V2 API因access_forbidden不可用（非密码问题）。全景脚本已改为三区展示(🔴亲自/🟡委派/📊统计)，cron已更新。新分类规则：4类亲自（制定方案/准备汇报/组织协调/终审签字），6类委派（数据核对/催办/填充/问询/流程/邮件）。
§
AI家庭教师(Edu-Hub) Flask 服务默认端口为 5003（app.py中 `port = int(os.environ.get('PORT', 5003))`），非 5002。edu-hub venv (~/edu-hub/ai-family-tutor/venv/) 全面依赖需单独 pip install（Flask/PIL 等均缺失）。pip install 在 WSL 默认超时失败，需用清华镜像 `-i https://pypi.tuna.tsinghua.edu.cn/simple`。
§
用户的多阶段设计管线偏好：每次会话可产出同一项目的多个独立交付物（原型→差距分析→设计说明书），每份交付物独立输出到飞书再触发下一阶段。两次触发顾问团评估的先后模式（先评原型再评差距分析）是用户的默认工作流，不是偶然。用户说"生成一份详细设计文档"是综合设计的标准信号，应合并所有前期产出物为单份综合说明书。无需等待用户明确说"继续"——每份交付物生成+上传后自动进入下一阶段。
§
AI家庭教师学生端设计确认(2026-05-21): ①首页打卡日历采用月视图网格(类家长端)，非7天圆点条；②首页增加拍照批改入口(📸拍试卷·改错题)和学习日历入口(📅)；③首页无错题本入口，错题本为各学科内独立模块；④语文学科页4卡片：每日练习/课程练习/单元练习/错题本；⑤补齐答题页(手写格子+选项+进度条)、学习日历页(月视图+当日详情)、拍照批改页(拍照/相册/历史)全部原型。
§
会议情报部处理规则（2026-05-21 用户要求）：会议转写内容只输出大致摘要，不输出完整文字记录。摘要包含：会议主题/时间/召集人、核心议题（1-3条）、结论/决策、待办事项、需关注重点。不包含：逐字转写、完整发言记录、会议过程描述。
§
2026-05-21: 学生端 v1.3 全量实施完成。新增 6 个模板、改造 2 个模板、新增 routes/student.py 中 8 条路由、新建 services/student_service.py 含 5 个服务函数。核心变更：首页月视图打卡日历+拍照批改入口+学科卡片、语文学科页4卡片入口、课程/单元/每日练习列表页、学习日历月视图、拍照批改页、答题页120px手写格增强。
§
Flask dual-process trap: when Flask dev servers are started twice on the same port, the kernel round-robins requests between old and new processes. The old process has stale Python route handlers. Template files (Jinja2) are read fresh from disk, so HTML changes appear correct, but route data differs. Fix: always `fuser -k <port>/tcp` before starting, then start via terminal(background=true). Check with `ss -tlnp | grep <port>` — expect exactly one PID. Captured in system-service-reliability skill.
§
学生端首页改造完成(2026-05-21): 底部5Tab导航(🏠📖🏆🪙👤)、7列月视图日历+图例、金色积分卡、学科3列grid(语文绿/数学蓝灰/英语粉灰)、统计栏。pad_base.html新增双导航模式支持(sidebar+底部Tab)，通过{% block bottom_tab %}和{% block sidebar %}覆盖切换。
§
Flask双进程陷阱(2026-05-21): 两个python进程绑定同一端口(5003)时，内核轮转分发请求，~50%命中旧进程。诊断: ss -tlnp | grep <port> 看到多PID。修复: fuser -k <port>/tcp 杀全部，单进程重启。已写入system-service-reliability skill。

---
Last synced: 2026-05-21 23:00